...
import { startOfWeek, addDays } from "./schedule";
import { generateChecklistForStage } from "./checklist";
 ...
 *
 * This module is the single source of truth for demo/seed data — both the
 * one-time `prisma/seed.ts` CLI script and the in-app "Reset Demo" action
 * (lib/demo-actions.ts) call seedDemoCompany() here, so there is exactly one
 * place that defines what the demo looks like. */
...
/** Seeds the full demo company — Mueller Construction Demo — with everything
 * a live walkthrough needs: 12+ active projects, historical closed jobs, the
 * hero project (Riverside Commerce Center), the estimator's next opportunity
 * (NorthPoint Distribution Center), and the 5 demo personas. Safe to call
 * standalone: it only ever touches rows under the one company it creates. */
export async function seedDemoCompany() {
  const company = await prisma.company.create({
    data: { name: "Mueller Construction Demo", slug: "mueller-construction-demo", isDemo: true },
  });
 ...
 * just that a companyId column exists. Its admin should never be able to
 * see any of the Mueller Construction Demo data, and vice versa. Kept
 * entirely separate from seedDemoCompany() so "Reset Demo" can wipe and
 * reseed the demo company without ever touching this one. */
export async function seedSecondCompany() {
